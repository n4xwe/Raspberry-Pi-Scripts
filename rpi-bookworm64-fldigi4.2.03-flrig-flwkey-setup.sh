#!/bin/sh
#install fldigi(4.2.03) w/Hamlib(4.5.5) flrig(2.0.05) flwkey(1.2.3) 
#N4XWE 01-10-2024
#Test Compiled on Raspberry Pi OS Bookworm dtd 2023-12-05 64-bit
...
tcl8.6-dev libspeex-dev libasound2-dev libgd-dev alsa-utils libgcrypt20-dev libpopt-dev libfltk1.3-dev libpng++-dev \
...
#Download the fldigi-4.2.03 source code from Sourceforge
wget https://sourceforge.net/projects/fldigi/files/fldigi/fldigi-4.2.03.tar.gz ||
  { echo 'Unable to download the fldigi source code file'; exit 1; }

#Extract the fldigi source code files
tar -xvzf fldigi-4.2.03.tar.gz ||
...
#Download the flrig 2.0.05 source code from Sourceforge
wget https://sourceforge.net/projects/fldigi/files/flrig/flrig-2.0.05.tar.gz ||
  { echo 'Unable to download the flrig source code file'; exit 1; }

#Extract the flrig source code files
tar -xvzf flrig-2.0.05.tar.gz ||
