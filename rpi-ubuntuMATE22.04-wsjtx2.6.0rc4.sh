#!/bin/sh
#install wsjt-x(2.6.0) Hamlib(4.5)
#N4XWE 11-28-2022
#Compiled on Ubuntu MATE 22.04 64-bit
...
wget -N https://sourceforge.net/projects/wsjt/files/wsjtx-2.6.0-rc4/wsjtx-2.6.0-rc4.tgz ||
  { echo 'Unable to download the WSJT-X source code file'; exit 1; }
  
#Extract the WSJT-X source code files
tar -zxvf wsjtx-2.6.0-rc4.tgz

#Create a directory for an indirect build of WSJT-X and make it the current directory
mkdir -p ~/src/WSJTX/wsjtx-2.6.0/build && cd ~/src/WSJTX/wsjtx-2.6.0/build
