...
sudo apt install -y git libfftw3-dev libgtk-3-dev libpulse-dev libasound-dev libpulse-mainloop-glib0 libusb-1.0-0-dev ||
...
#In the Makefile comment out the line GPIO_INCLUDE=GPIO
sed -i 's/GPIO_INCLUDE=GPIO/#GPIO_INCLUDE=GPIO/' ~/src/PIHPSDR/pihpsdr/Makefile
